...
def AE_IR(x,a,b,c):
    ...
    for i in range(len(xx)):
        ...
        yy[i] = ndsang.FeedForward(xx[i], Im, Tm, z)
    fig = plt.figure()
    cx = fig.subplots()
    # add code
    #cx.scatter(T10, T20, label='Data-set')
    cx.scatter(xx, yy, label='Data-set')
    #cx.plot(xx, yy, color='b', label='NN-Output')
    cx.plot(xx, gaussian(xx, Im, Tm, z), color='red', label='NN-Output')
...
plt.title('ANN approach for analyzing TL curve by FOK model')
